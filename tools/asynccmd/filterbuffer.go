...
func (b *Buffer) secretLinesToRedact(lineIdxToRedact int, matchMap map[int][]int) [][]byte {
	...
	secretIdxsByLineIdxToRedact := map[int][]int{}
	for secretIdx, lineIndexes := range matchMap {
		for _, lineIdx := range lineIndexes {
			secretIdxsByLineIdxToRedact[lineIdx] = append(secretIdxsByLineIdxToRedact[lineIdx], secretIdx)
		}
	}

	var secretChunks [][]byte
	for firstMatchingLineIdx, secretIndexes := range secretIdxsByLineIdxToRedact {
...
func redact(line []byte, ranges []matchRange) []byte {
	...
	for _, r := range ranges {
		...
		newLine := append([]byte{}, redacted[:first]...)
		newLine = append(newLine, RedactStr...)
		newLine = append(newLine, redacted[last:]...)
...
func (b *Buffer) redact(lines [][]byte, matchMap map[int][]int) [][]byte {
	...
	for idx, line := range lines {
		linesToRedact := b.secretLinesToRedact(idx, matchMap)
		...
		redacted[idx] = redact(line, ranges)
	}
