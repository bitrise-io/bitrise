...
// ValidateForShare ...
// Validates the step before stepman share process.
// Befor sharing, we should check if all required field exist,
// except step.PublishedAt & step.Source, which will generated by share process
func (step StepModel) ValidateForShare() error {
	...
	for _, input := range step.Inputs {
		var err error
		err = ValidateStepInputOutputModel(input, true)
		if err != nil {
			return err
		}
	}
	...
	for _, output := range step.Outputs {
		var err error
		err = ValidateStepInputOutputModel(output, true)
		if err != nil {
			return err
		}
	}
...
// If checkRequiredFields is set,
//  all required step fields will be checked (i.e. step and step.Inputs & step.Outputs envs).
// Without checkRequiredFields,
//  only step envs (i.e. step.Inputs & step.Outputs envs) will be validated (see: envman/models/models_methods.go/func (env EnvironmentItemModel) Validate() error),
//  and if these envs title property exist.
