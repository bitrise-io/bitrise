...
var errStepNotAvailableOfflineMode = fmt.Errorf("step not available in offline mode")

// Activate ...
func Activate(stepLibURI, id, version, destination, destinationStepYML string, updateLibrary bool, log Logger, isOfflineMode bool) error {
	collection, err := ReadStepSpec(stepLibURI)
	if err != nil {
		return fmt.Errorf("failed to read %s steplib: %s", stepLibURI, err)
	}

	return ActivateFromCollection(collection, stepLibURI, id, version, destination, destinationStepYML, updateLibrary, log, isOfflineMode)
}

func ActivateFromCollection(collection models.StepCollectionModel, stepLibURI, id, version, destination, destinationStepYML string, updateLibrary bool, log Logger, isOfflineMode bool) error {
	step, version, err := queryStep(collection, stepLibURI, id, version, updateLibrary, log)
	if err != nil {
		return fmt.Errorf("failed to find step: %s", err)
	}

	srcFolder, err := activateStep(collection, stepLibURI, id, version, step, log, isOfflineMode)
	if err != nil {
		if errors.Is(err, errStepNotAvailableOfflineMode) {
			availableVersions := ListCachedStepVersion(log, collection, stepLibURI, id)
...
func queryStep(stepLib models.StepCollectionModel, stepLibURI string, id, version string, updateLibrary bool, log Logger) (models.StepModel, string, error) {
	step, stepFound, versionFound := stepLib.GetStep(id, version)
	if (!stepFound || !versionFound) && updateLibrary {
		var err error
		stepLib, err = UpdateLibrary(stepLibURI, log)
		if err != nil {
...
func activateStep(stepLib models.StepCollectionModel, stepLibURI, id, version string, step models.StepModel, log Logger, isOfflineMode bool) (string, error) {
	route, found := ReadRoute(stepLibURI)
	if !found {
		return "", fmt.Errorf("no route found for %s steplib", stepLibURI)
	}

	stepCacheDir := GetStepCacheDirPath(route, id, version)
	...
	if err := DownloadStep(stepLibURI, stepLib, id, version, step.Source.Commit, log); err != nil {
...
func ListCachedStepVersion(log Logger, stepLib models.StepCollectionModel, stepLibURI, stepID string) []string {
...
func copyStepYML(libraryURL, id, version, dest string) error {
	route, found := ReadRoute(libraryURL)
	if !found {
	...
	stepCollectionDir := GetStepCollectionDirPath(route, id, version)
