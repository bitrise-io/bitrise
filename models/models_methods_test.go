...
`),
		...
		// Definition-side container validation
		{
...
step_bundles:
  test_bundle:
    execution_container: nonexistent
    steps:
    - script:
        inputs:
        - content: echo "test"
workflows:
  test:
    ...
    - bundle::test_bundle: {}
`),
			wantErr: "step bundle (test_bundle) has config issue: step bundle has container reference issue: undefined execution container (nonexistent) referenced",
		},
		{
...
workflows:
  test:
    ...
    - bundle::test_bundle: {}
`),
			wantErr: "step bundle (test_bundle) has config issue: step bundle has container reference issue: undefined service container (nonexistent) referenced",
		},
		{
...
workflows:
  test:
    ...
    - bundle::test_bundle: {}
`),
			wantErr: "step bundle (test_bundle) has config issue: step bundle has container reference issue: invalid execution container definition: empty container id",
		},
		{
			name: "Invalid bitrise.yml: step bundle definition has duplicate service container reference",
			config: createConfig(t, `
format_version: '25'
default_step_lib_source: https://github.com/bitrise-io/bitrise-steplib.git
containers:
...
workflows:
  test:
    ...
    - bundle::test_bundle: {}
`),
			wantErr: "step bundle (test_bundle) has config issue: step bundle has container reference issue: duplicate service container reference: redis",
		},
		{
...
workflows:
  test:
    ...
    - bundle::test_bundle: {}
`),
			wantErr: "step bundle (test_bundle) has config issue: step (script) has container reference issue: undefined execution container (nonexistent) referenced",
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
