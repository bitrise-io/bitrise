...
func validateDAGPipeline(pipelineID string, pipeline *PipelineModel, config *BitriseDataModel) error {
	for pipelineWorkflowID, pipelineWorkflow := range pipeline.Workflows {
		...
		isParallelizedWorkflow := pipelineWorkflow.Parallel != nil
		if isParallelizedWorkflow {
			if *pipelineWorkflow.Parallel < 1 {
				return fmt.Errorf("workflow (%s) defined in pipeline (%s) has invalid parallel value (%d), should be at least 1", pipelineWorkflowID, pipelineID, *pipelineWorkflow.Parallel)
			}

			for i := 1; i <= *pipelineWorkflow.Parallel; i++ {
				generatedWorkflowID := fmt.Sprintf("%s_%d", pipelineWorkflowID, i)
				if _, ok := config.Workflows[generatedWorkflowID]; ok {
					return fmt.Errorf("parallel workflow variant (%s) would be generated by workflow (%s) defined in pipeline (%s), but it is also defined as a workflow", generatedWorkflowID, pipelineWorkflowID, pipelineID)
				}
			}
		}
