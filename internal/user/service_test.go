...
func TestSignup_HappyPath(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/users/sign_up":
			...
			_, _ = io.WriteString(w, `<meta name="csrf-token" content="t1" />`)
		case "/users":
			if r.Header.Get("X-CSRF-Token") != "t1" {
				t.Errorf("X-CSRF-Token = %q, want %q", r.Header.Get("X-CSRF-Token"), "t1")
			}
			_ = json.NewDecoder(r.Body).Decode(&capturedBody)
			w.WriteHeader(http.StatusCreated)
			_, _ = io.WriteString(w, `{"slug":"u-aaa","email":"a@b.io","username":"alice","first_name":"Alice","last_name":"L","confirmed_at":null}`)
		default:
	...
	svc := NewService(c)
	acct, err := svc.Signup(context.Background(), SignupInput{
		Email: "a@b.io", Username: "alice", Password: "supersecret",
		FirstName: "Alice", LastName: "L",
	})
	if err != nil {
		t.Fatalf("Signup: %v", err)
	}
	if acct.Slug != "u-aaa" || acct.Email != "a@b.io" || acct.Username != "alice" {
		t.Fatalf("unexpected account: %#v", acct)
	}
	if acct.Confirmed {
		t.Fatalf("expected unconfirmed account on signup, got Confirmed=true")
	}
	wantUser := map[string]any{
		"email": "a@b.io", "username": "alice",
		"password": "supersecret", "password_confirmation": "supersecret",
		"first_name": "Alice", "last_name": "L",
	}
	gotUser, _ := capturedBody["user"].(map[string]any)
	for k, v := range wantUser {
		if gotUser[k] != v {
			t.Errorf("body.user.%s = %v, want %v", k, gotUser[k], v)
		}
	}
}

func TestSignup_422IncludesFieldErrors(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = io.WriteString(w, `{"errors":{"email":[{"error":"taken"}],"password":[{"error":"too_short"}]}}`)
	}))
	defer srv.Close()

	c, _ := webclient.New(srv.URL)
	svc := NewService(c)
	_, err := svc.Signup(context.Background(), SignupInput{Email: "x@y", Username: "x", Password: "p"})
	if err == nil {
