...
In previous versions of `bitrise-cli` the output envstore (which is a container for the step output environemnts)   
was not cleared after processing it's content. This leaded bitrise-cli to duplicate every output environment, which was generated by a step, after every next step run.

#### bash toolkit entry file support

So far this release bash-toolkit step's entry file was the hardcoded `step.sh`, from now these steps can specify the entry file path in the `step.yml`.
