...
func Test(t *testing.T) {
	// should be valid
	bitriseConfContent, _, err := generateBitriseYMLContent("App Title", "master")
	...
	bitriseConfModel, _, err := bitrise.ConfigModelFromYAMLBytes([]byte(bitriseConfContent))
	require.NoError(t, err)

	_, err = bitriseConfModel.Validate()
	require.NoError(t, err)
