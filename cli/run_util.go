...
func activateStepLibStep(stepIDData models.StepIDData, destination, stepYMLCopyPth string, isStepLibUpdated bool) (stepmanModels.StepInfoModel, bool, error) {
	...
	var versionConstraint stepmanModels.VersionConstraint
	if !isLatestVersionOfStep {
		var err error
		versionConstraint, err = stepmanModels.ParseRequiredVersion(stepIDData.Version)
		if err != nil {
			return stepmanModels.StepInfoModel{}, false, fmt.Errorf("activating step (%s) from source (%s) failed, invalid version specified: %s", stepIDData.IDorURI, stepIDData.SteplibSource, err)
		}
	}

	if (isLatestVersionOfStep || versionConstraint.VersionLockType == stepmanModels.MinorLocked || versionConstraint.VersionLockType == stepmanModels.MajorLocked) && !isStepLibUpdated {
