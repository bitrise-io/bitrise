...
- **`cmdutil.SilenceRootErrors` and `cli/cmdutil/picker`** are used only here.
...
- **Tests isolate once per package** via a `main_test.go` holding only
  ...
  elsewhere. In `session`, test files group by theme: `mutation_test.go` covers
  create/update/restore/terminate/delete, while `exec`, `logs`, `vnc` and `create`
  have their own.
...
- shared `cmdutil.ReadSecretInput` / `ReadPasswordInput` / `CheckPasswordStdinPiped`
