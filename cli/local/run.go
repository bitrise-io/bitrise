...
import (
	...
	"github.com/bitrise-io/bitrise/v3/cli/containermanager"
	...
	"github.com/bitrise-io/bitrise/v3/toolprovider"
...
point elsewhere.`,
  ...
  bitrise run primary --workflow primary`,
	...
	flags.String(cmdutil.OutputFormatKey, "", "Log format. Available values: json, console")

	flags.String(cmdutil.ConfigBase64Key, "", "base64 encoded config data.")
	flags.String(cmdutil.InventoryBase64Key, "", "base64 encoded inventory data.")
...
func run(cmd *cobra.Command, args []string) error {
	...
	if err != nil {
		if err == errWorkflowNotSpecified {
			...
			cmdutil.Failf("No workflow specified")
		} else if err == errUtilityWorkflowSpecified {
			printAboutUtilityWorkflowsText()
			cmdutil.Failf("Utility workflows can't be triggered directly")
		}
		cmdutil.Failf("Failed to process arguments: %s", err)
	}
	...
	runner := NewWorkflowRunner(*config, agentConfig, cmdutil.Tracker())
...
func (r WorkflowRunner) RunWorkflowsWithSetupAndCheckForUpdate() (int, error) {
	...
	if err := cmdutil.CheckUpdate(); err != nil {
...
func processArgs(cmd *cobra.Command, args []string) (*RunConfig, error) {
	workflowToRunID, _ := cmd.Flags().GetString(cmdutil.WorkflowKey)
	...
	inventoryEnvironments, err := cmdutil.CreateInventoryFromCLIParams(runParams.InventoryBase64Data, runParams.InventoryPath)
	if err != nil {
		return nil, fmt.Errorf("failed to create inventory: %s", err)
	}

	bitriseConfig, warnings, err := cmdutil.CreateBitriseConfigFromCLIParams(runParams.BitriseConfigBase64Data, runParams.BitriseConfigPath, bitrise.ValidationTypeMinimal)
	...
	enabledFiltering, err := cmdutil.IsSecretFiltering(secretFiltering, inventoryEnvironments)
	if err != nil {
		return nil, fmt.Errorf("failed to check Secret Filtering mode: %s", err)
	}

	enabledEnvsFiltering, err := cmdutil.IsSecretEnvsFiltering(secretEnvsFiltering, inventoryEnvironments)
	if err != nil {
		return nil, fmt.Errorf("failed to check Secret Envs Filtering mode: %s", err)
	}

	isSteplibOfflineMode := cmdutil.IsSteplibOfflineMode()
...
func registerRunModes(modes models.WorkflowRunModes) error {
	if err := cmdutil.RegisterCIMode(modes.CIMode); err != nil {
		return fmt.Errorf("failed to register CI mode: %s", err)
	}

	if err := cmdutil.RegisterPrMode(modes.PRMode); err != nil {
		return fmt.Errorf("failed to register PR mode: %s", err)
	}

	if err := cmdutil.RegisterSecretFiltering(modes.SecretFilteringMode); err != nil {
		return fmt.Errorf("failed to register Secret Filtering mode: %s", err)
	}

	if err := cmdutil.RegisterSecretEnvsFiltering(modes.SecretEnvsFilteringMode); err != nil {
