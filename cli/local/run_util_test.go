...
func TestIsSecretFiltering(t *testing.T) {
	...
	{
		set, err := cmdutil.IsSecretFiltering(pointers.NewBoolPtr(true), []envmanModels.EnvironmentItemModel{})
		require.NoError(t, err)
		require.True(t, set)

		set, err = cmdutil.IsSecretFiltering(pointers.NewBoolPtr(false), []envmanModels.EnvironmentItemModel{})
		require.NoError(t, err)
		require.False(t, set)
	}
	...
	{
		set, err := cmdutil.IsSecretFiltering(nil, []envmanModels.EnvironmentItemModel{
...
func TestIsPRMode(t *testing.T) {
	t.Run("Should be false for: prGlobalFlag: nil, prModeEnv: '', prIDEnv: ''", func(t *testing.T) {
		...
		pr, err := cmdutil.IsPRMode(nil, []envmanModels.EnvironmentItemModel{})
		require.NoError(t, err)
	...
	t.Run("Should be true for: prGlobalFlag: true, prModeEnv: '', prIDEnv: ''", func(t *testing.T) {
		...
		pr, err := cmdutil.IsPRMode(pointers.NewBoolPtr(true), []envmanModels.EnvironmentItemModel{})
		require.NoError(t, err)
...
`
		...
		pr, err := cmdutil.IsPRMode(nil, inventory.Envs)
		require.NoError(t, err)
		require.Equal(t, false, pr)
	})
...
`
		...
		pr, err := cmdutil.IsPRMode(pointers.NewBoolPtr(false), inventory.Envs)
		require.NoError(t, err)
		require.Equal(t, false, pr)
	})
...
`
		...
		pr, err := cmdutil.IsPRMode(pointers.NewBoolPtr(true), inventory.Envs)
		require.NoError(t, err)
		require.Equal(t, true, pr)
	})
...
func TestIsCIMode(t *testing.T) {
	t.Run("Should be false for: ciGlobalFlag: nil, ciModeEnv: 'false'", func(t *testing.T) {
		...
		ci, err := cmdutil.IsCIMode(nil, []envmanModels.EnvironmentItemModel{})
		require.NoError(t, err)
	...
	t.Run("Should be true for: ciGlobalFlag: true, ciModeEnv: 'false'", func(t *testing.T) {
		...
		ci, err := cmdutil.IsCIMode(pointers.NewBoolPtr(true), []envmanModels.EnvironmentItemModel{})
		require.NoError(t, err)
...
`
		...
		ci, err := cmdutil.IsCIMode(pointers.NewBoolPtr(false), inventory.Envs)
		require.NoError(t, err)
		require.Equal(t, false, ci)
	})
...
`
		...
		ci, err := cmdutil.IsCIMode(pointers.NewBoolPtr(true), inventory.Envs)
		require.NoError(t, err)
		require.Equal(t, true, ci)
	})
...
`
		...
		ci, err := cmdutil.IsCIMode(nil, inventory.Envs)
		require.NoError(t, err)
		require.Equal(t, true, ci)
	})
...
`
	...
	config, warnings, err := cmdutil.GetBitriseConfigFromBase64Data(configBase64Str, bitrise.ValidationTypeFull)
	require.NoError(t, err)
...
`
	...
	inventory, err := cmdutil.GetInventoryFromBase64Data(inventoryBase64Str)
	require.NoError(t, err)
